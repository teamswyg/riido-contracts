...
| Device/runtime liveness snapshot and stale projection | [`../20-domain/device-principal.md`](../20-domain/device-principal.md) | Daemon refreshes device/runtime liveness with an aggregated runtime snapshot every 5 seconds while running. Control-plane treats 20 seconds without a refreshed runtime snapshot as stale, projects device daemon availability and runtime availability as `offline`, projects runtime detection as `missing`, and excludes stale runtimes from newly derived `AgentRuntimeBinding`. Client generated code does not change; existing device/runtime read fields carry the projected values. |
