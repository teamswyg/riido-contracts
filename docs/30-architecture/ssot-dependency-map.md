...
| Task-thread cold collection, comment submit, progress stream, and stop operation semantics | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma annotation evidence at `node-id=153-15931` and normal task-screen evidence at `node-id=236-21379` | Control-plane implements `GET /v1/client/ai-agent/tasks/{task_id}/threads`, `POST /v1/client/ai-agent/tasks/{task_id}/comments`, `POST /v1/client/ai-agent/tasks/{task_id}/stop`, HTTP/SSE, and generated-client call chains. Daemon produces parsed progress batches with thread identity and consumes cancellation commands from SaaS, not UI clicks. Client owns scroll, hover, modal, animation, reply-input rendering, send-button state, task sidebar fields, and rendered thread composition. Infra acts only if a future topology/cost/evidence SSOT changes deployment requirements. |
...
| Onboarding template catalog and no-runtime branch | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma onboarding evidence at `node-id=42-3014`, `node-id=137-6746`, `node-id=138-7389`, and `node-id=164-30206` | Control-plane projects `agent_templates` through `GET /v1/client/ai-agent/bootstrap`. Client owns template row selection, direct-setting expansion, scroll, two-line ellipsis, workspace selection, and the decision to skip template selection when device/runtime data has no selectable runtime. Daemon supplies runtime liveness; infra acts only if template media/storage/search/topology changes appear. |
