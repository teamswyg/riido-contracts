...
| `instruction` is optional client-authored agent guidance text capped at 1000 characters | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture | Control-plane validates/stores/projects it, then snapshots it into `Assignment.agent_instruction` when an agent is assigned. Daemon consumes only that assignment snapshot for provider-specific prompt/native-config materialization. Infra acts only if a future storage/secret/media requirement appears. |
