...
| Task-thread cold collection, progress stream, and stop operation semantics | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma annotation evidence at `node-id=153-15931` | Control-plane implements `GET /v1/client/ai-agent/tasks/{task_id}/threads`, HTTP/SSE, and generated-client call chains. Daemon produces parsed progress batches with thread identity and consumes cancellation commands. Client owns scroll, hover, modal, animation, and rendered thread composition. Infra acts only if a future topology/cost/evidence SSOT changes deployment requirements. |
...
- Agent-setting docs may restate that Figma shows add/list/edit screens,
  ...
  must not hard-code model candidates as contract enum values before
  `Q-CON-006` is resolved. Only profile/configuration fields, create/update
  operations, editability, runtime binding, visibility, and `updated_at` are
  canonical contract facts today.
