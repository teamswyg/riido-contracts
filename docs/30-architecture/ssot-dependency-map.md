...
| Runtime model selection is assignment-created snapshot data | [`../20-domain/assignment-polling.md`](../20-domain/assignment-polling.md) plus [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) | Control-plane validates and stores an agent's runtime-scoped `model_id`, then copies it into `Assignment.model_id` when the agent is assigned. Daemon consumes only that snapshot when starting the provider runtime and must not infer model defaults from provider name, team id, Open API keys, or local environment. |
...
| Workspace-scoped AI Agent v2 API | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) and [`../20-domain/api-contract-projection.md`](../20-domain/api-contract-projection.md) | Contracts own `/v2/client/workspaces/{workspace_id}/ai-agent/...` and generated paths such as `riido.v2.aiAgent.agents.create`. Workspace list/create remains outside AI Agent; the selected `workspace_id` scopes agent create/read/assignment. v1 remains only as compatibility surface for existing UI tests. |
| DevicePrincipal enrollment and daemon credential boundary | [`../20-domain/device-principal.md`](../20-domain/device-principal.md) plus the API DSL policy metadata | Desktop registers account-owned devices from an established UserPrincipal session, stores the returned one-time device credential outside browser/webview JS, and launches daemon with `RIIDO_DEVICE_ID` / `RIIDO_DEVICE_SECRET`. Control-plane hashes and verifies the credential for daemon SaaS polling. Daemon sends `X-Riido-Device-ID` / `X-Riido-Device-Secret` and never consumes browser JWT. Client web generated code does not receive device secrets. |
| Device/runtime liveness snapshot and stale projection | [`../20-domain/device-principal.md`](../20-domain/device-principal.md) | Daemon refreshes device/runtime liveness with an aggregated runtime snapshot every 5 seconds while running, and the same snapshot carries daemon process facts for existing daemon-detail fields such as profile, PID, started-at, and uptime. Control-plane treats 20 seconds without a refreshed runtime snapshot as stale, projects device daemon availability and runtime availability as `offline`, projects runtime detection as `missing`, and excludes stale runtimes from newly derived `AgentRuntimeBinding`. Client generated code does not change; existing device/runtime and daemon-detail read fields carry the projected values. |
| AI Agent runtime progress messages are fixed, translated, integer-coded, append-only, and rendered before public SSE delivery | [`../20-domain/progress-message-catalog.md`](../20-domain/progress-message-catalog.md) and `../../progressmessage/catalog.dsl.riido.json` | Daemon parses structured provider telemetry or maps known legacy copy to `(message_code, message_args)`. Control-plane renders the fixed localized copy and keeps the public `message` string shape for existing frontend SSE/thread consumers. |
| Active assignment heartbeat and stale lease fencing | [`../20-domain/assignment-polling.md`](../20-domain/assignment-polling.md) | Daemon sends active assignment heartbeats every 5 seconds. Control-plane treats 20 seconds without refresh as stale, fails the active assignment before the same agent can claim later queued work, and must not let late heartbeat/event traffic revive the expired lease. Frontend generated code and visible thread APIs do not change for this runtime liveness policy. |
| Onboarding draft ordering is client-local until final workspace/runtime-scoped create | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md), [`../20-domain/api-contract-projection.md`](../20-domain/api-contract-projection.md), and the API DSL fixture, with Figma evidence at `node-id=432-46849` | Client/desktop may let users pick a fixture or type direct configuration before runtime/workspace selection. Control-plane still persists only final create requests that carry `workspace_id` through v2 URL scope and `runtime_id` through the request body. Daemon sees only the saved assignment/runtime snapshot. Infra is no-diff unless draft persistence, storage, or topology is introduced by a future SSOT. |
...
| Task participant AI Agent assign/unassign commands | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture | Control-plane implements `POST /v1/client/ai-agent/tasks/{task_id}/assignment` and `DELETE /v1/client/ai-agent/tasks/{task_id}/assignment`, preserves one active AI Agent per task, creates the initial `assignment_started` task-thread row, and projects unassign as `stopped_by_user_request`. Client owns human/agent section rendering and whether stopped rows are visually hidden. Daemon consumes the resulting SaaS assignment/stop state. Infra acts only if assignment durability/topology changes. |
| Generated assignment excludes team/OpenAPI key | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture | Control-plane may resolve task location internally from `task_id`, but generated clients do not send `team_id`, `teamId`, OpenAPI task-context paths, or Open API key transport such as `X-Workspace-Api-Key`. Agent records stay team-less, daemon polling receives only assignment snapshots, and infra treats team/OpenAPI-key config as outside generated-assignment deployment/smoke reasoning. |
...
| Runtime settings device/runtime read model plus device-bound and agent-bound daemon control/detail contracts | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma annotation evidence at `node-id=162-23090` | Control-plane implements `GET /v1/client/ai-agent/devices`, `GET /v1/client/ai-agent/devices/{device_id}/daemon`, `POST /v1/client/ai-agent/devices/{device_id}/daemon/start`, `POST /v1/client/ai-agent/devices/{device_id}/daemon/restart`, `POST /v1/client/ai-agent/devices/{device_id}/daemon/stop`, `GET /v1/client/ai-agent/agents/{agent_id}/daemon`, `POST /v1/client/ai-agent/agents/{agent_id}/daemon/start`, `POST /v1/client/ai-agent/agents/{agent_id}/daemon/restart`, `POST /v1/client/ai-agent/agents/{agent_id}/daemon/stop`, `device_runtime_snapshot`, and `device_daemon_status_changed`. The `내 기기` daemon row uses generated `riido.aiAgent.devices.daemon.*` with `device_id` from the device read model; explicit agent-mediated daemon access uses generated `riido.aiAgent.agents.daemon.*`. Daemon reads accepted SaaS commands and executes local lifecycle behavior; stop acceptance makes affected runtimes offline in the read model. Public agent access delegates indirect daemon/runtime execution to workspace members, while private agent access limits agent-bound daemon detail/control to admins and owners. Workspace admin can see same-workspace device/runtime rows required by RBAC or visible agent access, but cannot receive every persisted device from the control-plane snapshot. Client owns hover/modal/animation rendering. Infra acts only if a future topology/storage/evidence SSOT changes deployment requirements. |
...
| Runtime-scoped model catalog and agent `model_id` selection | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, resolving `Q-CON-006` from Figma `node-id=432-37336`, add-screen `node-id=134-6542`, and required-control evidence at `node-id=417-21803` / `node-id=432-35544` | Control-plane projects `RuntimeRecord.models`, validates create/update `model_id` against the selected runtime, defaults omitted values to the runtime default model, and projects saved `model_label`. Client may render the model dropdown as a non-empty required control backed by the default model. Daemon consumes only the already-authorized selected model for runtime execution. Infra acts only if a future deployment-backed model catalog/store/cache/index/evidence SSOT changes topology. |
| Agent setting add/list/edit fields, creation/update timestamps, and add-button eligibility presentation | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma section evidence at `node-id=432-37336`, add-screen evidence at `node-id=134-6542`, list/add-button evidence at `node-id=337-24001` / `node-id=337-24013`, and list-screen evidence at `node-id=432-35713` | Control-plane implements `POST /v1/client/ai-agent/agents`, selected authorized runtime validation, `created_at`/`updated_at` projection, immutable creation-time stamping, and mutation-time update refresh. Client owns save-button enablement, row/meatball edit entry, add affordance hide/show when the authorized device/runtime read model has no selectable runtime, form required-control presentation, delete confirmation, disabled edit tooltip/cursor behavior, no-description row layout, status-label copy/color, long-description presentation, timestamp formatting/tooltip, and dropdown rendering. Daemon consumes only assigned runtime/model values after upstream policy supplies them. Infra acts only if a future model catalog, storage, search, media, or evidence SSOT changes topology. |
| Fixture-created agent identity and lifecycle | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus [`../20-domain/api-contract-projection.md`](../20-domain/api-contract-projection.md), with Figma discussion evidence at `node-id=162-23468` / `node-id=162-23475` | Fixture-created agents are ordinary agents. Control-plane keeps submitted duplicate display names, returns a distinct `agent_id`, and uses normal update/delete/editability/RBAC behavior. Client renders names as display data, not identity. Daemon does not branch on fixture origin. Infra stays no-diff unless a future fixture registry, media, search, durability, or evidence axis appears. |
| Onboarding runtime selection, fixture catalog, direct setting, draft ordering, and no-runtime branch | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma onboarding evidence at `node-id=42-3014`, `node-id=137-6746`, `node-id=138-7389`, `node-id=162-23468`, `node-id=164-26969`, `node-id=164-30192`, `node-id=164-30206`, and `node-id=432-46849` | Control-plane projects `AgentOnboardingFixtureListResponse.fixtures` through `GET /v1/client/ai-agent/onboarding/fixtures` and v2 workspace-scoped duplicate routes, creates a normal agent through fixture/direct create routes, projects device/runtime liveness through device runtime reads, and validates the selected `runtime_id` through create/update mutations. Direct setting projects `이름` / `설명` / `지침` into the existing create request fields, not a separate command or fifth fixture. Figma `432:46849` changes only the client explanation/order: clients may collect a local agent draft before runtime/workspace selection, but durable v2 create still happens only after `workspace_id` and `runtime_id` are known. Client owns runtime radio rendering, `감지됨` / `감지 안 됨` labels, row dimming, fixture row selection, the `직접 설정` presentation row, disabled-next state before a fixture/direct path is selected, preview skeleton/popover rendering, direct-setting expansion, scroll, placeholders, two-line ellipsis, workspace selection/list scrolling/create-new entry points, all-disconnected provider-list rendering, the `시작하기` CTA, local draft state, and the decision to skip fixture selection when device/runtime data has no selectable runtime. The selected workspace id is passed to v2 AI Agent routes; workspace list/create itself remains client/team product surface. Daemon supplies runtime liveness/detection facts; infra acts only if fixture media/storage/search/topology changes appear. |
...
| Runtime prompt/native-config consumption | `riido-daemon` C4/C6 SSOTs | Contracts own the assignment snapshot field and instruction value semantics, not process execution or provider files. |
...
- `api-contract-projection.md` owns DSL/IR/OpenAPI mechanics. Control-plane may
  restate the generated fixture flow because it runs the local generator/test
...
- Generated client delivery docs may restate branch names, tag triggers,
  ...
  other than review handoffs, and they must not auto-merge client PRs.
- Desktop/control-plane/daemon docs may restate DevicePrincipal enrollment and
  credential injection as local execution detail. They must not change the
  ownership graph, replace the credential headers, or expose device secrets to
  generated frontend clients without updating `device-principal.md`.
- Control-plane/daemon/infra docs may restate that dynamic
  `AgentRuntimeBinding` projection replaces static deployment-secret bindings.
  They must not reintroduce `RIIDO_AI_SERVER_AGENT_BINDINGS_JSON` as the source
  of production daemon assignment without updating `device-principal.md` and
  `assignment-polling.md`.
...
- Onboarding docs may restate that Figma shows runtime choice, fixture choice,
  direct setting, scroll, description ellipsis, and no-installed-AI branches.
  They must not turn client presentation into new API commands. The fixture
