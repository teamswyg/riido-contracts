...
| Onboarding template catalog and no-runtime branch | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma onboarding evidence at `node-id=42-3014`, `node-id=137-6746`, `node-id=138-7389`, `node-id=164-30192`, and `node-id=164-30206` | Control-plane projects `agent_templates` through `GET /v1/client/ai-agent/bootstrap` and device/runtime liveness through `GET /v1/client/ai-agent/devices`. Client owns template row selection, direct-setting expansion, scroll, two-line ellipsis, workspace selection/list scrolling/create-new entry points, all-disconnected provider-list rendering, the `시작하기` CTA, and the decision to skip template selection when device/runtime data has no selectable runtime. Daemon supplies runtime liveness; infra acts only if template media/storage/search/topology changes appear. |
