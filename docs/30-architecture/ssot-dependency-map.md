...
| Onboarding runtime selection, fixture catalog, direct setting, and no-runtime branch | [`../20-domain/ai-agent-policy.md`](../20-domain/ai-agent-policy.md) plus the AI Agent API DSL fixture, with Figma onboarding evidence at `node-id=42-3014`, `node-id=137-6746`, `node-id=138-7389`, `node-id=162-23468`, `node-id=164-26969`, `node-id=164-30192`, and `node-id=164-30206` | Control-plane projects `AgentOnboardingFixtureListResponse.fixtures` through `GET /v1/client/ai-agent/onboarding/fixtures` and v2 workspace-scoped duplicate routes, creates a normal agent through fixture/direct create routes, projects device/runtime liveness through device runtime reads, and validates the selected `runtime_id` through create/update mutations. Direct setting projects `이름` / `설명` / `지침` into the existing create request fields, not a separate command or fifth fixture. Client owns runtime radio rendering, `감지됨` / `감지 안 됨` labels, row dimming, fixture row selection, the `직접 설정` presentation row, disabled-next state before a fixture/direct path is selected, preview skeleton/popover rendering, direct-setting expansion, scroll, placeholders, two-line ellipsis, workspace selection/list scrolling/create-new entry points, all-disconnected provider-list rendering, the `시작하기` CTA, and the decision to skip fixture selection when device/runtime data has no selectable runtime. The selected workspace id is passed to v2 AI Agent routes; workspace list/create itself remains client/team product surface. Daemon supplies runtime liveness/detection facts; infra acts only if fixture media/storage/search/topology changes appear. |
