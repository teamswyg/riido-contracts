...
| Q-CON-006 | Runtime model catalog | Should the AI Agent setting screen's model dropdown be owned as a canonical runtime capability catalog, a control-plane client sub-DSL read model, or a daemon-reported runtime fact? | Do not add `model_id` to agent configuration until ownership is settled. The current Figma `node-id=164-50215` dropdown labels (`Sonnect 4.6 (기본값)`, `Opus 4.7`, `Haiku 4.5`, `Opus 4.6`, `Opus 3`) are planning/UI evidence, not contract enum values. The contract owns runtime binding, profile fields, visibility, instruction, editability, and `updated_at` today. |
