...
func verifyCounts(model model) error {
	...
	checks := map[string][2]int{
		...
		"provider health statuses":  {len(model.ProviderHealthStatuses), m.ExpectedProviderHealthStatus},
		"provider diagnostic codes": {len(model.ProviderDiagnosticCodes), m.ExpectedProviderDiagnosticCode},
