...
func verifyManifest(m manifest) error {
	...
	if m.Package == "" || len(m.ScopeRules) == 0 {
		return errors.New("manifest executable package and scope rules are required")
	}
	if len(m.Invariants) == 0 {
		return errors.New("manifest invariants are required")
	}
